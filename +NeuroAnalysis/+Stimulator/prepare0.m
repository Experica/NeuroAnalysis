...
logfilepath = fullfile(filedir,[filename,'.logfile']);
d = dir(logfilepath);
if ~isempty(d)
    islogfile=true;
...
if ~isempty(dataset) && (strcmp(dataset.exptId,'Hartley') || strcmp(dataset.exptId,'randomBar') || strcmp(dataset.exptId,'BO')|| strcmp(dataset.exptId,'NaturalScene')) && strcmp(dataset.eventSource,'spike2')
    % Parse spike data
    ex = parseevent(ex);
    ...
    function [ ex ] = parseex( ex )
        ...
        if isfield(ex.raw,'log')
            ...
            elseif strcmp(dataset.exptId,'randomBar')
                ex.ID = 'randomBar';
            elseif strcmp(dataset.exptId,'MV')
                ex.ID = 'MV';
            end
            
            log = ex.raw.log;
            ex.EnvParam.DisplayRefreshRate = log.frate;
            if strcmp(ex.ID,'Hartley')
                ts = fieldnames(log);
            ...
            if strcmp(ex.ID,'randomBar') || strcmp(ex.ID,'MV')
               % So far, only attach log file without any processing.
               % The processing needed is to organize the ctc based on the random stimulus sequence.
                % condidx=[];
                % ctc=[];
                % cond=[];
            ...
            switch daqType
                ...
                case 'spike2'
                        ...
                        if chn>0
                            ...
                            if strcmpi(ex.DisplayType,'crt')
                                [di,dv,pd]=parsecrtdigitalinanalog(anaData,0.02,0.38,0.1);
...
%                             ex.spike2.digital(2).value=[dv(1),dv(1),dv(2:end)];
                ...
                if ex.nTrial == length(ex.(daqType).digital(1).time)/2
                    if (strcmp(ex.ID,'Hartley') || strcmp(ex.ID,'randomBar') || strcmp(ex.ID,'MV')) && ex.PreICI==0 && ex.SufICI==0
