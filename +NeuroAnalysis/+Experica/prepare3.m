function [ex] = prepare3(ex,dataset)
%PREPARE2 Prepare version 3 of Experica.Command data format
...
%% Try Parse Experiment Extended Parameter
if ~isempty(ex.ExtendParam)
    ex.ExtendParam = tryparseparamstruct(ex.ExtendParam);
end
...
if isfield(ex.CondTest,'Event') && isfield(ex.CondTest,'SyncEvent')
    ...
    if ~isempty(dataset) && isfield(dataset,'digital') && ~isempty(dataset.digital)
        if ex.EventSyncProtocol.NChannel==1 && ex.EventSyncProtocol.NEdgePEvent==1
