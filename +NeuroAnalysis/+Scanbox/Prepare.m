...
% filepath = 'F:\AF7\2P_data\U005\metaFiles\AF7_005_008.analyzer';  % for test
...
if (~isempty(spike2File))
    spike2MatFile = dir(fullfile(spike2File.folder,[spike2File.name(1:end-4), '.mat']));
    isSyncdata = true;
    dataset.eventSource = 'spike2';
else
    spike2MatFile = struct([]);
    isSyncdata = false;
    dataset.eventSource = '';
end
